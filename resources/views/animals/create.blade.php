...
@section('content')
    <div class="container mt-4">
        ...
        <form action="{{ isset($animal) ? route('animals.update', $animal->id) : route('animals.store') }}" method="POST"
            class="row g-4 bg-light p-4 rounded-4 shadow-sm border border-success bg-opacity-25">
            @csrf
            @if (isset($animal))
                @method('PUT')
            @endif
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="animal" id="animal" value="{{ old('animal', $animal->animal ?? '') }}"
                    class="form-control rounded-3 shadow-sm" placeholder="Digite o nome do animal" required>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="diet" id="diet" class="form-select rounded-3 shadow-sm" required>
                    <option value="" disabled {{ old('diet', $animal->diet ?? '') == '' ? 'selected' : '' }}>Selecione uma
                        dieta</option>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="habitat" id="habitat" value="{{ old('habitat', $animal->habitat ?? '') }}"
                    class="form-control rounded-3 shadow-sm" placeholder="Digite o habitat do animal" required>
            </div>

            <!-- Quantidade -->
            <div class="col-md-3">
                <label for="amount" class="form-label fw-semibold text-success">Quantidade:</label>
                <input type="number" name="amount" id="amount" value="{{ old('amount', $animal->amount ?? '') }}"
                    class="form-control rounded-3 shadow-sm" placeholder="Digite a quantidade" required>
            </div>

            <!-- Origem -->
            <div class="col-md-3">
                <label for="origin" class="form-label fw-semibold text-success">Origem:</label>
                <input type="text" name="origin" id="origin" value="{{ old('origin', $animal->origin ?? '') }}"
                    class="form-control rounded-3 shadow-sm" placeholder="Digite a origem" required>
            </div>
